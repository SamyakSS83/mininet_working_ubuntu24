...
# For Ubuntu, use the specific conda Python path
if [ "$DIST" = "Ubuntu" ]; then
    SPECIFIC_PYTHON="/home/threesamyak/miniconda3/bin/python"
    if [ -x "$SPECIFIC_PYTHON" ]; then
        if $SPECIFIC_PYTHON -c "$PRINTVERSION" |& grep 'major=2'; then
            PYTHON=$SPECIFIC_PYTHON; PYTHON_VERSION=2; PYPKG=python
        elif $SPECIFIC_PYTHON -c "$PRINTVERSION" |& grep 'major=3'; then
            PYTHON=$SPECIFIC_PYTHON; PYTHON_VERSION=3; PYPKG=python3
        fi
    fi
fi

# If specific Python not found or not Ubuntu, try standard detection
